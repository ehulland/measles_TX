...
#########################################
## Changing reporting day by 24 hours ###
#########################################
...
#add one day to the dates, as if everything were reported with 24 hours delay:
dat2[,Date:=Date+1]
for(i in 1:nrow(grid)){
  si<-grid$si[i]
  r0<-grid$r0[i]
  ...
  #get all estimates for March 11th (most recent day of data in main analysis): 
  median<-data.frame(si=si, r0=r0)
  median$median_rt<-r_samp[Date=='2025-03-11', R_median]
  median$R_025<-r_samp[Date=='2025-03-11', R_025]
  median$R_975<-r_samp[Date=='2025-03-11', R_975]
  
  median$VC_median<-r_samp[Date=='2025-03-11',VC_median]
  median$VC_025<-r_samp[Date=='2025-03-11', VC_975]
  median$VC_975<-r_samp[Date=='2025-03-11', VC_025]
...
#R(t), VC, and coverage gap (from a threshold of 93%) for our six scenarios for most recent day of data
setDT(medians)
medians2<-medians[!is.na(r0)]
medians2[,gap:=93-VC_median]
medians2[,gap_u:=93-VC_025]
...
png("~/Measles_TX_barchart_vc_sens1day.png")
ggplot(medians2) +
...
png("~/Measles_TX_linechart_vc_sens1day.png", width=650, height=600)
